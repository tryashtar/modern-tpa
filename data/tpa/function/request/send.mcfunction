...
$execute if data storage tpa:data pending[{here:1b,from:"$(name)",to:"$(sender)"}] run return run function tpa:pending/accept {from:"$(name)",to:"$(sender)",accept:"tp $(sender) $(name)"}
...
$data modify storage tpa:data pending append value {here:0b,accept:"tp $(sender) $(name)",from:"$(sender)",to:"$(name)",from_index:{index:$(sender_index)},to_index:{index:$(index)},menu_cancel:{label:{translate:"tpa.menu.cancel.button",fallback:"%s %s %s",with:[{atlas:"gui",sprite:"friends/cancel",shadow_color:0},{player:{name:"$(name)"}},"$(name)"]},tooltip:{translate:"tpa.menu.cancel.tooltip",fallback:"Cancel request to teleport to this player"},action:{type:"run_command",command:"trigger tpcancel_$(name)"}},menu_accept:{label:{translate:"tpa.menu.accept.button",fallback:"%s %s %s",with:[{atlas:"gui",sprite:"friends/accept",shadow_color:0},{player:{name:"$(sender)"}},"$(sender)"]},tooltip:{translate:"tpa.menu.accept.tooltip",fallback:"Allow this player to teleport to you"},action:{type:"run_command",command:"trigger tpaccept_$(sender)"}},menu_reject:{label:{translate:"tpa.menu.reject.button",fallback:"%s %s %s",with:[{atlas:"gui",sprite:"friends/reject",shadow_color:0},{player:{name:"$(sender)"}},"$(sender)"]},tooltip:{translate:"tpa.menu.reject.tooltip",fallback:"Disallow this player to teleport to you"},action:{type:"run_command",command:"trigger tpreject_$(sender)"}}}
